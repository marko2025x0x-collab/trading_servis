...
import { ShieldCheck, UserPlus, Mail, Lock, User, ArrowLeft, CheckCircle2, AlertCircle, Sparkles } from 'lucide-react';
...
export default function RegisterPage() {
  ...
  const handleDemoSignUp = () => {
    ...
    setStatusFeedback({
      type: 'success',
      message: 'Акаунт успішно зареєстровано в Демо Режимі! Перенаправлення в термінал...',
    });
    setTimeout(() => {
    ...
    <main className="min-h-screen bg-[#050811] text-[#E2E8F0] flex flex-col items-center justify-center p-4 font-neo-mono selection:bg-[#00FF9D]/30">
      ...
      <div className="relative w-full max-w-md bg-[#090E1C] border border-[#00FF9D]/30 rounded-[3px] p-6 shadow-2xl neo-hud-bracket space-y-6">
        ...
        <div className="text-center space-y-2">
          <Link href="/pro-dashboard?demo=true" className="inline-flex items-center gap-2 text-xs text-[#00FF9D] hover:underline mb-2 font-bold">
        ...
        {/* Instant Demo Registration Button */}
        <button
          type="button"
          onClick={handleDemoSignUp}
          className="w-full py-2 bg-gradient-to-r from-emerald-950/80 to-slate-900 hover:from-emerald-900/80 hover:to-slate-800 text-[#00FF9D] font-bold rounded-[2px] transition-all flex items-center justify-center gap-2 border border-emerald-500/40 text-xs"
        >
          <Sparkles className="w-3.5 h-3.5" />
          <span>ШВИДКА ДЕМО РЕЄСТРАЦІЯ У ТЕРМІНАЛ</span>
        </button>
